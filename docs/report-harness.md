...
- Modal 冷启动的 `303` 只在 HTTPS、同源 `.modal.run`、同路径且查询参数唯一为非空
  `__modal_attempt_token` 时视为同一物理尝试；transport 随后用一次 `GET` 取回结果，
  不重发 `POST`。其他重定向、跨域地址、附加参数和二次重定向全部拒绝。
- 已进入读取阶段但超时的专家请求不自动重试；只有明确断联、写入失败、协议中断、
  无有效结果或可重试 HTTP 状态才进入受控重试，避免单并发端点同时生成重复结果。
...
被捕获后静默降级为稀疏结果。控制器执行合同版本5拒绝直接续跑旧合同，
包含语义问题严重度下限与规则摘录引用否决；不把新判定规则套用到旧运行的恢复过程。
...
补证读取接受规范 `evidence:UUID` 或能唯一匹配当前冻结记录的裸 UUID；
输出、访问记录及分页绑定统一为规范引用，重复别名和快照外ID仍拒绝。
事故表单字段继续使用 `accident:/JSON-Pointer`。
...
知识资产的 `source_kind` 区分 `rule_excerpt` 与 `source_chunk`；规则摘录提供
同来源正文片段索引，但该索引不计作已读。模型应继续读取完整条款、前提例外及施行
信息，再独立判断具体适用性；程序不会仅凭事故日期自动作法律适用结论。
...
候选仍将规则摘录用于断言引用时，控制器增加 `citations` 重大问题并将引用检查
标为失败，进入原有受控修订流程。免责声明或模型通过结论不能覆盖该确定性失败。
问题持续存在时保留稳定身份和闭合条件；移除摘录引用不等于自动关闭问题，
仍由独立审查核实完整正文和适用性。旧资产的 `#rule#` 标识同样不能绕过此检查。
...
报告SSE每0.5秒轮询新事件；空闲约15秒发送不带序号的注释心跳，
避免长时间模型等待触发代理空闲超时。心跳不计入持久事件，
也不改变客户端实际断连时的取消语义。
...
角色循环在接受最终响应前还检查两类可由模型自行修正的约定，逐项反馈类型、字段路径与说明，
与格式修复共用两次上限，耗尽后以 `needs_review/invalid_role_response` 停止，约定本身不放宽：

- `candidate_contract`：候选版本号、问题回应只针对台账中未关闭问题，以及发布终检中只涉及候选稿
  自身的规则（断言须有来源、knowledge 断言须引知识、正文定位有效、义务处置完整）。
  这些问题在审查前交还生成者，不再等审查费用花掉后才终止；发布前仍用同一规则终检。
- `source_not_read`：每个候选轮次清空原文访问记录，生成者的断言与问题回应、审查者须核对的
  必要事实与候选知识引用，都必须是本轮实际取得（知识须完整读完）的原文；模型可先调用读取工具
  再重交或删去该引用。修订稿沿用上一版引用也须重新读取。

审查结果的结构与台账规则（覆盖检查、五类检查、问题跟进）仍在合并台账后终检，失败即停止。
运行因校验停止时，文档另存 `terminal_detail`（异常类别、字段路径和程序固定提示，
不含模型输出或事故原文）供排查，不经公开接口返回；显式恢复时清除。
...
旧报告接口（含流式）必须登录，匿名请求返回401；`input_path`、`video_path` 这类服务器文件路径可指向任何用户的资料，
仅限管理员使用，普通用户返回403，普通用户通过上传接口提交材料。
