...
`runtime_factory.py`，当前提供显式注入的工程运行工厂，尚不等于生产路由已开启。
...
远端生成、审查和嵌入限定为当前受支持的 OpenRouter 路径，发包时带与预留报价对应的
供应商单价过滤并禁止回退；它限制收费路由，不是输出 token 上限。温度与表达详略
沿用原端点配置并纳入运行摘要，模型不得覆盖。专家响应在进入持久账本前去除可识别的
...
未指定 token 预算时按已核验模型容量和默认24次物理请求生成内部预留；
