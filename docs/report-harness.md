...
取证约定同样在角色循环内、接受最终响应前检查：每个候选轮次清空原文访问记录，
生成者的断言与问题回应、审查者须核对的必要事实与候选知识引用，都必须是本轮实际取得
（知识须完整读完）的原文。不满足时逐项反馈来源ID与字段路径（类型 `source_not_read`），
模型可先调用读取工具再重交或删去该引用；与格式修复共用两次上限，耗尽后以
`needs_review/invalid_role_response` 停止。约定不放宽，修订稿沿用上一版引用也须重新读取。
