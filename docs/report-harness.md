...
可使用证据/运行数据接口；
但 `online_enabled` 仍必须为 false，公开生产入口的模型执行保持关闭。旧配置缺少此配置组时行为不变。
合成角色只用于控制流测试，不能证明真实报告质量。

当前开发路径支持人工文字补证、冻结快照和受控的本地知识查询；
计账 transport 已在隔离回环 HTTP 中集成验证；另有须明确授权的隔离真实开发入口，生产工厂尚未接入在线角色，
未完成的能力明确拒绝，不假装已执行。
...
- 公开生产入口即使授权成功，outbound 仍返回 `outbound_transport_unavailable`。
...
按模型完整容量保守预留；未知费用继续占用额度。`MoneyGuard`在物理请求层用SQLite
