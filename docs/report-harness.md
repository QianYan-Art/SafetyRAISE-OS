...
历史版本因此把第一份已保存的单工具响应误当候选而进入
`needs_review/invalid_review_or_candidate` 时，只对无候选、无未知请求且日志摘要完整的
记录提供 `can_resume_protocol`。原页面复用继续按钮，不新建报告；恢复仍检查owner、
版本、原授权、预算和活动运行约束，不允许重试未知请求，也不改写原模型结果或摘要。
一般格式错误、质量审查否决以及已发布、已取消的终态都不能借此恢复。
