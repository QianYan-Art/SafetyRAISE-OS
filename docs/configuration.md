...
启用在线模式还要求数据库迁移、代码校验清单与只读发布批准匹配。代码、批准、知识或合同
...
- 视觉 / 嵌入 / 报告模型按「每用户能力配置」（`user_capability_configs`）解析：用户在前端「模型接入设置」里填 `url + key + model`，留空时仅嵌入回退系统默认，视觉/报告必须由普通用户自行填写（管理员留空则用系统默认，便于测试）。
